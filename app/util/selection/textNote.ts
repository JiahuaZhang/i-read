...
import { default_highlight_colors } from '~/routes/ePub.$fieldId.$.id';
...
export class TextHighlight extends Highlight {
  ...
  hydrate({ highlighter, doc, container }: HighlighterInfo) {
    ...
    textHighlight.elements = range.getNodes([], node => (node as Element)?.className?.includes(textHighlight.note.className)) as Element[];
